...
def make_ccsd_trial(mf):
    ...
    return driver, ccpy_total_energy(driver)
...
def run_afqmc(staged, *, hs_decomposition: str):
    af = Afqmc(staged, hs_decomposition=hs_decomposition)
...
if __name__ == "__main__":
    ...
    ccsd_driver, e_ccsd = make_ccsd_trial(mf)
    staged = stage_from_ccpy(ccsd_driver, mf, order=2,
                             chol_cut=1.0e-14, verbose=False)

    e_ccsdt = run_ccsdt_reference(mf)
#    e_charge, err_charge = run_afqmc(staged, hs_decomposition="charge")
    e_spin, err_spin = run_afqmc(staged, hs_decomposition="spin")

    print("\nCCSD trial/reference:")
    print(f"  E_CCSD  = {e_ccsd}")
    print(f"  E_CCSDT = {e_ccsdt}")
    print("\nCharge decomposition with UCISD trial:")
#    print(f"  E = {e_charge} +/- {err_charge}")
#    print(f"  Delta vs CCSDT = {e_charge - e_ccsdt}")
    print("\nSpin decomposition with UCISD trial:")
    print(f"  E = {e_spin} +/- {err_spin}")
    print(f"  Delta vs CCSDT = {e_spin - e_ccsdt}")
